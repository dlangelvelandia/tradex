...
def paginate(q, page, per_page):
    ...
    if per_page is None: per_page = 50
...
def listar_usuarios():
    ...
    if qtext:
        like = f"%{qtext}%"
        q = q.filter((Usuario.email.ilike(like)) | (Usuario.nombre_completo.ilike(like)))
    if rol_nombre:
...
def crear_ruta():
    ...
    # Parseo de fecha/hora (SQLite exige objetos date/time)
    fecha_programada_val = None
    if d.get("fecha_programada"):
        try:
            fecha_programada_val = datetime.strptime(d["fecha_programada"], "%Y-%m-%d").date()
        except ValueError:
...
def ruta_geojson(ruta_id):
    ...
    for p in paradas:
        ...
        features.append({
            ...
            "properties": {
                "id": p.id, "orden": p.orden, "titulo": p.titulo,
                "direccion": p.direccion, "ventana_inicio": p.ventana_inicio,
                "ventana_fin": p.ventana_fin, "notas": p.notas
            }
        })
    ...
    return jsonify({
        "ruta": {"id": r.id, "codigo": r.codigo, "nombre": r.nombre, "estado": r.estado},
...
def mis_rutas():
    ...
    data = [{"id": r.id, "codigo": r.codigo, "nombre": r.nombre, "estado": r.estado} for r in q.all()]
    return jsonify(data)
...
def actualizar_estado_ruta(ruta_id):
    ...
    # Opcional: limitar a valores válidos
    validos = {"planificada", "en_progreso", "completada", "cancelada"}
    if nuevo not in validos:
        return jsonify(error=f"Estado inválido. Válidos: {', '.join(validos)}"), 400
