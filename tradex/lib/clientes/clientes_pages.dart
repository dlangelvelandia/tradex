...
// Creación de rutas (simplificada) y Rutas pendientes.
  ...
  Widget build(BuildContext context) {
    Widget item({
      ...
      return Padding(
        ...
        child: InkWell(
          onTap: () { if (!sel) Navigator.pushReplacementNamed(context, route); },
      ...
      child: Column(
        ...
        children: [
          Row(children: const [
            ...
            SizedBox(width: 10),
            Text('TRADEX LOGISTIC', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
          ]),
...
class _CreacionRutaPageState extends State<CreacionRutaPage> {
  ...
  int? _conductorId;
  Map<String, dynamic>? _vehiculo; // vehículo del conductor (auto-detectado)

  bool _cargandoCombos = false;
  ...
  void dispose() {
    ...
    for (final p in _paradas) { p.dispose(); }
    super.dispose();
  }

  Future<void> _cargarConductores() async {
    setState(() => _cargandoCombos = true);
    try {
      final cons = await Api.listarUsuariosPorRol('Conductor');
      setState(() => _conductores = cons);
    } catch (_) {
      // ignore
    } finally {
      if (mounted) setState(() => _cargandoCombos = false);
    }
  }
  ...
  Future<void> _onSeleccionConductor(int? id) async {
    ...
    try {
      ...
      setState(() => _vehiculo = v); // puede ser null si no tiene
    } catch (_) {
      // ignore
    }
  }
  ...
  void _eliminarParada(int i) => setState(() => _paradas.removeAt(i).dispose());
  ...
  Future<void> _pickFecha() async {
    ...
    if (sel != null) {
      _fechaCtrl.text = '${sel.year.toString().padLeft(4, '0')}-${sel.month.toString().padLeft(2, '0')}-${sel.day.toString().padLeft(2, '0')}';
      setState(() {});
    }
  }
  ...
  Future<void> _pickHora() async {
    ...
    if (sel != null) {
      _horaIniCtrl.text = '${sel.hour.toString().padLeft(2, '0')}:${sel.minute.toString().padLeft(2, '0')}';
      setState(() {});
    }
  }
  ...
  Future<void> _guardar() async {
    ...
    if (_paradas.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Agrega al menos una parada')));
      return;
    }
    ...
    for (int i = 0; i < _paradas.length; i++) {
      ...
      if (lat == null || lng == null) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Parada #${i + 1}: lat/lng inválidos')));
        return;
      }
    }
    ...
    try {
      final meta = <String, dynamic>{
        if (_pesoCtrl.text.trim().isNotEmpty) 'peso_kg': double.tryParse(_pesoCtrl.text.trim()),
        if (_volumenCtrl.text.trim().isNotEmpty) 'volumen_m3': double.tryParse(_volumenCtrl.text.trim()),
        if (_tipoCargaCtrl.text.trim().isNotEmpty) 'tipo_carga': _tipoCargaCtrl.text.trim(),
      }..removeWhere((_, v) => v == null);

      final clienteId = Session.userId!;
      final creadoPor = Session.userId!;

      // Crear ruta (estado fijo: planificada; sin prioridad/hora fin)
      final r = await Api.crearRuta(
        codigo: _genCodigo(),
        nombre: _nombreCtrl.text.trim(),
        descripcion: _descripcionCtrl.text.trim().isEmpty ? null : _descripcionCtrl.text.trim(),
        estado: 'planificada',
        fechaProgramada: _fechaCtrl.text.trim().isEmpty ? null : _fechaCtrl.text.trim(),
        horaInicio: _horaIniCtrl.text.trim().isEmpty ? null : _horaIniCtrl.text.trim(),
        clienteId: clienteId,
        creadoPor: creadoPor,
      ...
      for (int i = 0; i < _paradas.length; i++) {
        ...
        await Api.agregarParada(
          ...
          titulo: p.tituloCtrl.text.trim().isEmpty ? null : p.tituloCtrl.text.trim(),
          direccion: p.dirCtrl.text.trim().isEmpty ? null : p.dirCtrl.text.trim(),
          notas: p.notasCtrl.text.trim().isEmpty ? null : p.notasCtrl.text.trim(),
        );
      }

      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Ruta #$rutaId creada')));
      _resetForm();
    } on ApiError catch (e) {
      if (!mounted) return;
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No se pudo crear la ruta')));
    } finally {
  ...
  void _resetForm() {
    ...
    _vehiculo = null;
    for (final p in _paradas) { p.dispose(); }
    _paradas..clear()..add(_ParadaModel());
    setState(() {});
  }
              ...
              child: Column(
                ...
                children: [
                  Text('Hola, ${Session.name ?? "Cliente"}',
                      style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: kTexto)),
                    ...
                    child: Padding(
                      ...
                      child: Form(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Datos de la ruta', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                                ...
                                Expanded(
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      suffixIcon: IconButton(icon: const Icon(Icons.calendar_month), onPressed: _pickFecha),
                                    ),
                                  ),
                                ),
                                const SizedBox(width: 12),
                                Expanded(
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      border: const OutlineInputBorder(),
                                      suffixIcon: IconButton(icon: const Icon(Icons.schedule), onPressed: _pickHora),
                                    ),
                                  ),
                                ),
                              ],
                            ...
                            // Carga
                            const Text('Datos de la carga (opcional)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                            const SizedBox(height: 12),
                            Row(
                              children: [
                                Expanded(
                            ...
                            // Asignación
                            const Text('Asignación (opcional)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                            const SizedBox(height: 12),
                            Row(
                              children: [
                                Expanded(
                                  child: DropdownButtonFormField<int>(
                                    ...
                                    items: _cargandoCombos
                                        ? const []
                                        : _conductores.map((c) {
                                            return DropdownMenuItem(
                                              value: (c['id'] as num).toInt(),
                                              child: Text(c['nombre_completo'] ?? c['email'] ?? '#${c['id']}'),
                                            );
                                          }).toList(),
                                    onChanged: (v) => _onSeleccionConductor(v),
                                  ),
                                ),
                                const SizedBox(width: 12),
                                Expanded(
                            ...
                            Row(
                              ...
                              children: [
                                const Text('Paradas', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                                OutlinedButton.icon(onPressed: _agregarParada, icon: const Icon(Icons.add), label: const Text('Agregar parada')),
                              ],
                            ),
                            const SizedBox(height: 8),
                            ...
                            for (int i = 0; i < _paradas.length; i++)
                              _ParadaCard(
                                index: i,
                                model: _paradas[i],
                                onDelete: _paradas.length > 1 ? () => _eliminarParada(i) : null,
                              ),
...
class _ParadaCard extends StatelessWidget {
  ...
  const _ParadaCard({required this.index, required this.model, this.onDelete});
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Row(
              children: [
                Text('Parada #${index + 1}', style: const TextStyle(fontWeight: FontWeight.w700)),
                const Spacer(),
            ...
            Row(
              children: [
                Expanded(
                  child: TextFormField(
                    controller: model.latCtrl,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(labelText: 'Latitud', border: OutlineInputBorder()),
                    validator: (v) => (v == null || double.tryParse(v) == null) ? 'Lat inválida' : null,
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: TextFormField(
                    controller: model.lngCtrl,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(labelText: 'Longitud', border: OutlineInputBorder()),
                    validator: (v) => (v == null || double.tryParse(v) == null) ? 'Lng inválida' : null,
                  ),
                ),
              ],
            ),
...
// ================== Página: Rutas pendientes (igual que antes) ==================
...
class _RutasPendientesPageState extends State<RutasPendientesPage> {
  ...
  Future<void> _cargar() async {
    ...
    try {
      final resp = await Api.listarRutasCliente(Session.userId!, estado: _filtroEstado);
      final data = (resp['data'] ?? []) as List;
      setState(() => _rutas = data.cast<Map>().map((e) => e.cast<String, dynamic>()).toList());
    } finally {
  ...
  void _abrirMapa(int rutaId, String titulo) {
    showDialog(
      ...
      builder: (_) => Dialog(
        ...
        child: SizedBox(width: 900, height: 560, child: _MapaRutaDialog(rutaId: rutaId, titulo: titulo)),
      ),
    );
  }
              ...
              child: Column(
                ...
                children: [
                  Text('Hola, ${Session.name ?? "Cliente"}',
                      style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: kTexto)),
                  const SizedBox(height: 8),
                  const Text('Rutas pendientes', style: TextStyle(fontSize: 36, fontWeight: FontWeight.w800, color: kTexto)),
                  const SizedBox(height: 14),
                  ...
                  Row(
                    children: [
                      ...
                      if (_filtroEstado != null)
                        TextButton(onPressed: () { setState(() => _filtroEstado = null); _cargar(); }, child: const Text('Quitar filtro')),
                    ],
                  ),
                  const SizedBox(height: 12),
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (_, i) {
                                ...
                                return _RowRuta(
                                  ...
                                  vehiculo: r['vehiculo_id']?.toString() ?? '-',
                                  onMapa: () => _abrirMapa((r['id'] as num).toInt(), r['nombre'] ?? r['codigo'] ?? 'Ruta'),
                                );
                              },
...
class _RowRuta extends StatelessWidget {
  ...
  const _RowRuta({required this.codigo, required this.nombre, required this.estado, required this.conductor, required this.vehiculo, required this.onMapa});
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(flex: 2, child: Text(codigo, style: const TextStyle(fontWeight: FontWeight.w700))),
          Expanded(flex: 3, child: Text(nombre)),
          Expanded(flex: 2, child: Text(estado, style: TextStyle(color: color, fontWeight: FontWeight.w700))),
          Expanded(flex: 3, child: Text(conductor)),
          Expanded(flex: 2, child: Text(vehiculo)),
          SizedBox(
            width: 160,
            child: OutlinedButton.icon(onPressed: onMapa, icon: const Icon(Icons.map_outlined), label: const Text('Ver mapa')),
          )
        ],
      ),
    );
...
class _MapaRutaDialog extends StatefulWidget {
  final int rutaId;
  final String titulo;
  const _MapaRutaDialog({super.key, required this.rutaId, required this.titulo});
  @override
...
class _MapaRutaDialogState extends State<_MapaRutaDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final poly = (linea?['geometry']?['coordinates'] as List?)?.map((c) {
      final l = c as List;
    ...
    return Column(
      children: [
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
          decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: kBordeSuave))),
          child: Row(
            children: [
        ...
        Expanded(
          child: _cargando
              ...
              : FlutterMap(
                  ...
                  children: [
                    TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png', userAgentPackageName: 'com.example.tradex_web'),
                    if (poly != null && poly.isNotEmpty)
                      PolylineLayer(polylines: [Polyline(points: poly, strokeWidth: 4.0)]),
                    MarkerLayer(
                      markers: [
                              ...
                              child: Column(
                                children: [
                                  Container(
                                    ...
                                    child: Text('${props['orden']}. ${props['titulo'] ?? 'Parada'}',
